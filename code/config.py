...
# config.py for RFQ Review and CRM Quote Automation Agent
import os
import logging
...
class Config:
    ...
    # Sets for LLM kwargs logic (for future LLM calls)
...
def _initialize_config():
    ...
    kv_secrets = {}
    if USE_KEY_VAULT:
        kv_secrets = Config._load_keyvault_secrets()
    ...
    # List of all config variables required for this agent
    CONFIG_VARIABLES = [
        # General
        "ENVIRONMENT",
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        elif USE_KEY_VAULT and var_name in kv_secrets:
            value = kv_secrets[var_name]
        else:
            value = os.getenv(var_name)
